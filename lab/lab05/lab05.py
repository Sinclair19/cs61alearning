...
def add_trees(t1, t2):
    ...
    if (is_leaf(t1)) or (is_leaf(t2)):
        return tree(label(t1) + label(t2), branches(t1) if branches(t1) else branches(t2))
    tmp = []
    t1branch = branches(t1)
    t2branch = branches(t2)
    for it in zip(t1branch, t2branch):
        tmp.append(add_trees(it[0], it[1]))
    if len(t1branch) > len(t2branch):
        tmp.append(t1branch[len(t2branch)])
    if len(t2branch) > len(t1branch):
        tmp.append(t2branch[len(t1branch)])
    return tree(label(t1) + label(t2), tmp)
