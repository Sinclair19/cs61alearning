...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        temp = other_card.defense
        other_card.defense = other_card.attack
        other_card.attack = temp
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opponent_card_deck_iter = iter(opponent.deck.cards[:]) # create a copy of a list, iterate the copy and delete elemnet from the origin
        try:
            while True:
                elem = next(opponent_card_deck_iter)
                if elem.attack == other_card.attack and elem.defense == other_card.defense:
