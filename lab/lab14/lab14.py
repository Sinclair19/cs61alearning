...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def num_splits_inner(s, n):
        if len(s) == 0:
            return 1 if abs(n) <= d else 0
        return num_splits_inner(s[1:], n + s[0]) + num_splits_inner(s[1:], n - s[0])
    return num_splits_inner(s, 0) // 2
...
class CheckingAccount(Account):
    ...
    "*** YOUR CODE HERE ***"
    def deposit_check(self, object):
        if object.deposited == True:
            print('The police have been notified.')
        else:
            if object.payable == self.holder:
                object.deposited = True
                return self.deposit(object.balance)
            else:
                print('The police have been notified.')
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edits, counts = helper_align(skeleton_idx + 1, code_idx + 1)
            possibilities.append((skel_char + edits, counts))
        # Insert
        edits, counts = helper_align(skeleton_idx, code_idx + 1)
        edits = "+[" + code_char + "]" + edits
        possibilities.append((edits, counts + 1))
        # Delete
        edits, counts = helper_align(skeleton_idx + 1, code_idx)
