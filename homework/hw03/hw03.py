...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) * upper_bound(y) > 0, 'Error: divisor cann\'t be 0'
...
def check_par():
    ...
    True
    """
    r1 = interval(5, 4) # Replace this line!
...
def quadratic(x, a, b, c):
    ...
    low = lower_bound(x)
    up = upper_bound(x)
    if medium > low and medium < up:
        return interval(min(f(medium), f(low), f(up)), max(f(medium), f(low), f(up)))
    else:
        return interval(min(f(low), f(up)), max((f(low), f(up))))
