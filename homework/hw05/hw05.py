...
def store_digits(n):
    ...
    while temp >= 10:
        temp = temp // 10
        divisor *= 10
    return Link(temp, store_digits(n - temp * divisor))
...
def is_bst(t):
    ...
    if len(t.branches) > 2:
        return False
    if len(t.branches) == 2:
        if bst_max(t.branches[0]) > t.label or t.label >= bst_min(t.branches[1]):
            return False
...
def preorder(t):
    ...
    temp_list = [t.label]
    for branch in t.branches:
        temp_list.extend(preorder(branch))
    return temp_list
...
def path_yielder(t, value):
    ...
    yield from paths
    '''
    # this way can't deal with situation like finding path to 2 in [0,2,1,2], it will only return [0,2]
    temp_list = []
    label = t.label
    def helper(t, value):
        if t.is_leaf() or t.label == value:
            return [value] if t.label == value else None
        for branch in t.branches:
            path = helper(branch, value)
            if path:
                if t.label == label:
                    temp_list.append([t.label] + path)
                else:
                    return [t.label] + path
    helper(t,value)
