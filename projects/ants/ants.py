...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        if exit != None and exit.entrance == None:
            exit.entrance = self
        # END Problem 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place # start from the place of the ant
        distance = 0
        for _ in range(self.min_range): #set the start place to match the min_range
            if place != None and place.entrance != None:
                place = place.entrance
                distance += 1
            else:
                return None
        while place != beehive and distance <= self.max_range:
            if place.bees == []:
                place = place.entrance
                distance += 1
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        amount_damage = amount
        if self.armor - amount <= 0:
            amount_damage += self.damage
        for bee in self.place.bees[:]:        
            bee.reduce_armor(amount_damage)
...
# END Problem EC
    ...
    # BEGIN Problem EC
    existed = False
    buffed_ant = [] # list to store ants being buffed
    implemented = True   # Change to True to view in the GUI
    # END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.existed == False:
            self.true_queen = True
        else:
            self.true_queen = False
        QueenAnt.existed = True
    ...
    def action(self, gamestate):
        ...
        if self.true_queen == True:
            ...
            while place != None:
                if place.ant != None and place.ant not in self.buffed_ant:
                    place.ant.damage *= 2
                    self.buffed_ant.append(place.ant)
                place = place.exit   
        else:
        ...
        # END Problem EC
    def remove_from(self, place):
        '''override the remove_from method to let the true queen unremovable'''
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.backward == True and self.place.entrance.name != 'Hive':
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if isinstance(other, ContainerAnt) or self.contained_ant != None:
            return False
        return True
        # END Problem Optional 2
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem Optional 4
    "*** YOUR CODE HERE ***"
    previous_action = bee.action # store previous action
    def hooked_action(gamestate):
        nonlocal length
        if length >0:
            status(previous_action, bee)(gamestate) # when length >0 take modified action
            length -= 1
        else:
            if status == make_scare:
                bee.backward = False
            previous_action(gamestate) # when in modified action, manually run previous action one time
            bee.action = previous_action # rebind action to the normal one
    bee.action = hooked_action # rebind action method to hooked one above
    # END Problem Optional 4
        ...
        while current_place != beehive:
            if current_place.ant != None:
                if current_place.ant is not self:
                    Insects_dic[current_place.ant] = distance
                if not current_place.ant.can_contain: # check if this ant can contain
                    if current_place.ant.contained_ant != None and current_place.ant.contained_ant is not self:
                        Insects_dic[current_place.ant.contained_ant] = distance # add the insect in container
